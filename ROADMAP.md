...
- [ ] Extend Jungfrau with audited interchange sequences and measured terrain; establish tunnel-aware vertical semantics for the Jungfraujoch section and source-supported Eiger Express cabin behaviour before adding 3D motion. Audit additional branches and operating dates separately.
...
- [x] Add separate King’s Cross, St Pancras domestic mainline/high-speed and St Pancras Thameslink boards. The shared Tube hero offers an explicit rail-area selector; exact rail entries retain their own source IDs. Area changes preserve the station and clock, dismissal preserves the area, and movement seeking opens the selected rail service. Document shared Tube counts and the Eurostar exclusion.
