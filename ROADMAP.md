...
The [national-study thesis](docs/NATIONAL-STUDY.md) and [technical feasibility research](docs/NATIONAL-DATA-FEASIBILITY.md) are research artifacts. London's reusable data tooling and the other studies' evidence practices give that proposal a stronger starting point; no national edition has been implemented by this update. Later on 13 September the thesis gained a [second axis](docs/NATIONAL-STUDY.md#a-second-axis-power-weather-and-light), power alongside weather and light, and the [vision](docs/VISION.md#the-view-from-altitude--13-september-2026) recorded the view-from-altitude principle behind it. Work on an England-wide study is commencing; what it will teach remains, literally, to be seen. On 14 September the first national hour of bus positions was recorded through Underfall's collector and a first settled electricity day was captured and placed; the same evening the first complete civil day of national buses, TfL arrival predictions, cycle-hire docks and Swiss trip updates was scheduled on the CI Mac; both are recorded in the [feasibility report](docs/NATIONAL-DATA-FEASIBILITY.md) and the [power brief](docs/POWER.md). On 15 September the study gained a private repository, [england](https://github.com/emmettl/england). The slug is a working name, not a title; no hosting route, catalogue entry or number follows from it. The repository holds the composition and a typed, reconciled contract for Underfall's compiled national bus day. Acquisition remains in Underfall for now. Most of Underfall's recording code serves the series rather than Bristol, so it was extracted with its history into a private [motionstudies-recorder](https://github.com/emmettl/motionstudies-recorder) repository on 15 September. On 16 September the recording host was switched to the recorder: it is deployed and smoke-tested there, its nightly, close and normalise loops have replaced Underfall's, and the first sweep recovered the 15 September national bus day, which an over-tight upload limit had refused. Collectors started before the switch finish from the old checkout; moving the recording directory and merging Underfall's removals remain. The extraction record lists the moved files, the modules both repositories still hold and the remaining steps. That night the host went further: the recordings moved to MiniMax, a dedicated 2 TB volume, and a supervisor run by launchd replaced the shell loops at 00:09 UTC on 17 September, with the switch's capture gaps kept rather than backfilled. Since 19 September it records five feeds (the national bus archive, TfL arrivals and docks, Swiss trip updates and BODS disruption situations), uploads each closed day through its own R2 ledger and publishes daily bus-quality analytics. Underfall merged its removals the same day. The manual air, power and road captures now reserve space in the same local ledger as the live feeds. The first fresh civil day to be verified end to end closes on 19 September. See the recorder's [cutover record](https://github.com/emmettl/motionstudies-recorder/blob/main/docs/EXTRACTION.md#recording-host-cutover) and [host operations](https://github.com/emmettl/motionstudies-recorder/blob/main/docs/HOST-OPERATIONS.md).
