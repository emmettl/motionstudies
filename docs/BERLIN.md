...
**Catalogue status:** source-pinned interactive proof and Ostkreuz relative interchange study implemented in an independent public repository; unnumbered, with measured rail/interchange heights still to source.

**Implementation:** [Umlauf repository](https://github.com/emmettl/umlauf) · [Data contract](https://github.com/emmettl/umlauf/blob/main/docs/DATA.md) · [Validation](https://github.com/emmettl/umlauf/blob/main/docs/VALIDATION.md) · [Next work](https://github.com/emmettl/umlauf/blob/main/docs/ROADMAP.md).
...
All Change's reusable flat-route visibility and rendering-work fixes are committed upstream in [4c86b50](https://github.com/emmettl/motionstudies/commit/4c86b501b7abbd1d136e5f5a9cb4fa6b59022d82) and now published as alpha.3 after the successful [trusted release](https://github.com/emmettl/motionstudies/actions/runs/34155462468). Berlin now installs exact alpha.3 registry pins and uses flat routes with quiet ground. Fourteen source/model tests and twenty desktop/phone checks pass for the upgraded build. Its software-rendered phone sample averaged 60 fps in the Ring and Ostkreuz, 34 fps in crossings and 32 fps in circulation; physical-device and p95 frame-time acceptance remain open.

The approved [private Sites review](https://umlauf-berlin-review.vndh2vk2v4.chatgpt.site) is deployed as version 4 from [Umlauf b1e6ea6](https://github.com/emmettl/umlauf/commit/b1e6ea6306cc1f3e0ccd1a27e8d56258a34559fd). Source/history upload and private deployment were explicitly approved; access is owner-only. The [publication review](https://github.com/emmettl/umlauf/blob/main/docs/REVIEW.md) records source rights and remaining editorial decisions. GitHub Pages was initially unavailable under the private repository plan; source visibility was subsequently changed with explicit approval, as recorded below. The title remains provisional and the edition unnumbered.

The approved [public GitHub Pages edition](https://emmettl.github.io/umlauf/) publishes source build `b97f1b1` from [successful checks](https://github.com/emmettl/umlauf/actions/runs/34159579680). The private repository plan initially blocked Pages, so the separate [deployment repository](https://github.com/emmettl/umlauf-pages) contains only compiled site assets and approved datasets. The user subsequently explicitly approved making the Umlauf source repository and its full Git history public; that visibility change is complete. [Pages deployment](https://github.com/emmettl/umlauf-pages/actions/runs/34161660665) succeeded, and all eleven served build files match the CI artifact byte for byte. See the [publication record](https://github.com/emmettl/umlauf/blob/main/docs/PUBLISHING.md).

The canonical site subsequently moved to `/umlauf/` at the user’s request. The public source repository now publishes through its manual checked Pages workflow ([deployment run](https://github.com/emmettl/umlauf/actions/runs/34163250377), source `1df5b28`); the former `/umlauf-pages/` address redirects to it.

The author approved the geographic/circulation transition. Direction controls now isolate S41, S42 or both without changing the clock or crossing family. In the authored diagram amber S41 runs outside mint S42, with shared source platforms retained and the complete Ring framed around its origin. This spacing is illustrative; geographic and timetable bytes are unchanged. Fifteen source/model tests and twenty-two desktop/phone browser checks pass locally; the opening remains within its 575 KiB gzip gate. The continuous city-to-Ostkreuz transition is now implemented: the map approaches the station before blending into its authored relative-level view, with reversible travel and a preserved clock. Additional station names emerge automatically at closer zoom; Aa controls train labels independently. Seventeen source/model and thirty desktop/phone browser checks cover the updated edition, including interrupted travel, slow/failed source recovery and reduced motion. Travel uses elapsed time so a slow renderer cannot stretch its duration; reversal coverage uses controlled time. Data artifacts are unchanged. Next: author review of the complete sequence and physical-device performance acceptance.
