...
4. *Publication shapes.* One JSON per slice is about 55 KB in the window. One gzipped evidence pack per operator totals 18 MB for the hour; the largest, TfL, is 3.2 MB compressed for 7,682 vehicles, so a full day of TfL would exceed the 25 MiB file limit and packs must be per operator per hour, about 8,800 files a day, inside the 20,000-file limit.

Steps 5 and 6, retention expiry and cost admission, need the object store and remain unrun. The decision this run supports: the deduplicated Parquet table with capture references is the durable national layer; archives are held for the rolling window; the published descent is slices, tile-to-member indexes and per-operator-per-hour packs, all static.
