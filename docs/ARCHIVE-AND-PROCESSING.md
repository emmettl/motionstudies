...
The following estimates compare data volumes, not permission to expand retention. Apply the [shared budget, traffic and retention controls](COST-CONTROL.md): initially propose a 30-day rolling window and at most 1,000 GB in R2 across all stored layers, including a finite allocation for selected study evidence. The Mac mini’s 2 TB volume, confirmed on 16 September, supports longer local retention independently of that cloud window and ceiling. Size local windows per source and layer using measured compressed bytes per day and the available local allocation; retain compact normalised history longer where useful, with explicit evidence-availability records after raw expiry. A year of retained data remains a comparison case until its local footprint is measured and budgeted. Public delivery should use the direct static-asset path described there; public R2 reads would introduce visitor-dependent operation charges.
...
**The day compiler exists, 14 September 2026** (Underfall `data:bus:packs`; [evidence](evidence/national-day-packs-2026-09-14.json)). It reads one UTC day's hourly partitions and nothing else, and writes the descent as decided: one JSON of cells per five-minute slice, one JSON per slice from cell to member vehicles, one gzipped pack per operator per day split by a sample budget where the day exceeds the part size, an index from operator and vehicle to part, and a manifest listing hours present and missing, exclusions, every file and its size. Samples are chosen by observation time within the day, which keeps stale last-known positions carried from the previous day out of the day they were received in; the manifest counts them. On the recorded hour's two partitions it wrote 634 files and 24 MB in six seconds: 2.0 million samples, 26,432 vehicles, 362 operators, packs at 9.2 bytes per sample with TfL at 3.3 MB, and member indexes at 315,000 entries. Projected to a day that is about 660 files and 450 MB, as estimated above. The day is built in a working directory and renamed into place, so a failed rebuild leaves the published day untouched. Later the same evening both gaps closed: Underfall's close-out (`data:day:close`, swept by `scripts/close-loop.sh` on the recording host) normalises a stopped journal's hours, compiles the published day per date, uploads the date's partitions and published tree through the ledger and records completeness, and its pruner (`data:store:prune`) removes raw captures older than a window only where the hour is normalised and the date uploaded; and `@motionstudies/core/domain/published-day` reads the format in the browser, with a lab specimen over a trimmed copy of this day.

**Operating the recorder, 14 September 2026.** Underfall's `data:status` reports every store under a work root from what the recorders and normalisers already write: journal progress, failed attempts grouped by reason, bytes raw and stored with the encoding at rest, normalised hours, ledger usage and disk; failed attempts now carry their error message, so a rate limit shows as such in the journal. Compression at rest reached the recording host the same evening as a packed build of the shared data package installed over the published alpha, ahead of the first full feed days; the coordinated alpha replaces it.
