...
All four shared packages were published at coordinated version `0.1.0-alpha.28` under `next` on 18 September 2026 from main `bf344f6` ([PR #33](https://github.com/emmettl/motionstudies/pull/33)). The [trusted release workflow](https://github.com/emmettl/motionstudies/actions/runs/35405153681) passed verification and publication with registry artifact checks; the main package check and catalogue deployment passed on the same commit. Shared validation before merge: 424 unit tests across 90 files, typecheck, lint and architecture boundaries; the packed consumer passed public imports, declarations, Node tooling, the lab build and 100 Chromium/iPhone WebKit browser specimens with two skips; the publish dry run packed all four packages at the release version.

Since alpha.27 this release adds the two core modules above and, from [PR #31](https://github.com/emmettl/motionstudies/pull/31), the `data` package's feed observability, incident history, feed observer and recorder evidence modules with a UK service-day change; `web` carries a small timeline-scrubber stylesheet change. `three` is unchanged beyond the coordinated version.

Underfall adopted the exact alpha.28 pins on its `codex/running-time-research` branch at `5528ff0`, moving from alpha.23 and so also taking alpha.24–27. Against the registry installation it passed 145 feed tests, lint, the production build and 14 Playwright regressions (Monday recording, vehicle hero, playback lifecycle) on desktop Chrome and iPhone WebKit. The route-field and route-events modules resolve from the installed package; no Underfall code uses them yet.
