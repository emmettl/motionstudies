...
The shared release and its package/catalogue checks succeeded. Umlauf, All Change, Manifest and Norikae main completed their publication workflows, and Local / Express completed its validation workflow. Correspondances, Zugunruhe and the final lab/catalogue follow-up also completed successfully. Norikae's development branch has no push-triggered workflow.

Gleislicht's package/fixture checks succeeded, but its national timetable job stopped at the existing road-coverage requirement: today's generated timetable achieved 92.80%, below the required 95%. The [job log](https://github.com/emmettl/gleislicht/actions/runs/34712505305/job/103604325262) reports that no artifacts were written. All of that run's package, browser and regional jobs subsequently passed. The national timetable failure was addressed in the follow-up below.

## Road-cache follow-up

Gleislicht commit [`4f5aeeb`](https://github.com/emmettl/gleislicht/commit/4f5aeebbb016a5782f3d8d4bc0f4e19b13a2cbfa) rebuilds the national PostBus cache from a full week of GTFS release `20260909`. The original Tuesday-only cache was missing 548 Saturday patterns affecting 1,766 trips. Saturday coverage is now 99.79% (previously 92.80%) and Sunday 99.79% (previously 91.12%); all seven days reach 99.79–99.80% with the original matching tolerances and 95% gate. Per-day artifact audits, all 1,396 unit tests, typecheck, lint, architecture, build and bundle checks passed. The preparation command now accepts multiple service-day snapshots and rejects mixed timetable releases or conflicting platform coordinates.

The [validation receipt](https://github.com/emmettl/gleislicht/blob/4f5aeebbb016a5782f3d8d4bc0f4e19b13a2cbfa/data/postbus-road-refresh-20260912.json) records source hashes and every day's coverage and size results. The user's working checkout contains the fix with unrelated edits preserved. The [fresh deployment run](https://github.com/emmettl/gleislicht/actions/runs/34714790904) is running; production completion has not yet been confirmed. No additional npm release was required.
