...
| [WebTRIS normalization](../packages/data/src/webtris.mjs) | Report labels, nullable measurements, length classes and duplicate/conflict audits retained through aggregate conversion. | Recorder now owns bounded WebTRIS pagination/capture and immutable evidence releases (16 September 2026). Bristol consumes the release offline. London's existing hourly light/heavy conversion still requires an explicit migration review. |
...
- **BODS:** current national response bytes and identifiers, conversion losses, usable London AVL coverage, current account limits, and any historical raw-location offering beyond the early-2026 research finding. The account exists through Underfall, whose collector, store and windowing already handle bounded recordings. Measured 13 September 2026: the unauthenticated datafeed answers 401 and the unfiltered datafeed with a valid key answers 403, so the API serves filtered subsets only. The whole country is the unauthenticated bulk archive at `/avl/download/bulk_archive`, which redirects once to a signed, short-lived URL for a dated SIRI-VM ZIP on a separate download host; GTFS-RT has the same shape. Underfall's `data:bus:probe` command measures one archive in full, reporting compressed and expanded bytes, activity/vehicle/operator counts, repeated samples, missing journey references, observation ages and a per-day projection without retaining records or the signed URL; its report sets the collector's limits before a national window is recorded. Underfall's collector now has a `--national` mode (13 September 2026) that captures the archive through the shared source store, keeps the ZIP as the immutable raw object, expands and decodes it in a stream, and writes national snapshots on the existing datafeed contract; a national hour fits its defaults, while a full day needs compressed or delta snapshot storage that is not yet implemented.

  **First national measurement, Sunday 13 September 2026, 20:41 UTC** (Underfall [probe report](https://github.com/emmettl/underfall/blob/main/docs/evidence/bods-national-probe-2026-09-13.json)): one archive of 1.46 MB compressed and 18.2 MB expanded held 16,989 activities from 200 operator codes, each a distinct vehicle with no repeated sample within the response. The archive is a last-known-position snapshot: 3,948 positions were within 30 seconds of the provider timestamp, 9,111 were older than an hour and the oldest was a day old, so the research literature's duplicate share arises across successive requests and a distinct vehicle is not an active one. All 6,728 TfL records lack a dated journey reference; outside London it is present. 75 positions fall outside Great Britain, 72 at 0°,0°. Linear projection: 4.2 GB compressed or 52 GB expanded per day at 30-second intervals, before a weekday-peak factor. This replaces the arithmetic scenarios above for the bus archive.

  **Second measurement, Monday 14 September 2026, 10:38 UTC** (Underfall [probe report](https://github.com/emmettl/underfall/blob/main/docs/evidence/bods-national-probe-2026-09-14.json)): 2.33 MB compressed and 30.0 MB expanded; 26,926 vehicles from 365 operator codes; 14,841 positions within 30 seconds of the provider timestamp and 18,981 within a minute; 4,162 older than an hour; 7,661 without a dated journey reference, 7,644 of them TfL. Linear projection: 6.7 GB compressed or 87 GB expanded per day at 30-second cadence. A weekday morning therefore carries roughly 19,000 vehicles reporting within a minute, which is the working count of lights in the national field; a full day of compressed archives fits an ordinary disk, and expanded snapshots do not.

  **First national hour recorded, Monday 14 September 2026, 11:42–12:42 UTC** (Underfall [summary](https://github.com/emmettl/underfall/blob/main/docs/evidence/bods-national-hour-2026-09-14.json)): 120 archive captures at 30-second cadence, none failed, no gaps. 26,995 distinct vehicles produced 1,990,226 distinct samples, about 74 per vehicle, a mean reporting interval of 49 seconds; fresh positions per capture averaged 17,059. Sixty-one percent of each capture's samples were new, alternating between 52 and 69 percent on a one-minute rhythm, so the research literature's duplicate share is confirmed as a property of re-served last-known positions. Archives were 280 MB for the hour; uncompressed normalised snapshots were about 1.8 GB. The bounded recording the decision gates asked for now exists, and the first complete national day is scheduled on the CI Mac for 15 September 2026, archive-only under a capacity ledger. The [archive-and-processing proof](ARCHIVE-AND-PROCESSING.md#first-implementation-proof) has since run on it: replay from retained objects, a deduplicated Parquet layer of 25.8 MB for the hour, exact reconciliation of five-minute tiles, a group of 847 vehicles in one London cell, and a two-millisecond cold lookup of one vehicle's evidence.
...
### Roads ownership update — 16 September 2026

Existing aggregate-road and WebTRIS contracts cover the new recorder release without a package version change. Recorder owns source acquisition, normalization, model versions and budgets; England owns national selection/coverage/composition; Bristol owns local M32 sections and urban countline/crossing studies. One existing NTIS receiver is retained, pending compatible models and contract verification. New recorder commands are bounded manual operations; since 19 September they reserve their retained bytes in the same local capacity ledger as the continuous collector, but they remain unscheduled and outside its retention and upload lifecycle. The council VivaCity importer preserves multimodal classes and availability as an adapter artifact; a shared public contract should follow verified interval semantics and a second consumer. Underfall `docs/ROADS-INFRASTRUCTURE.md` records the detailed source audit and implementation; recorder `docs/ROADS.md` defines commands and limits.
