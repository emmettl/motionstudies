...
The shared package and lab extraction is complete. At extraction, editions installed exact `0.1.0-alpha.2` npm releases: All Change (London), Correspondances (Paris), Gleislicht (Switzerland), and the private Local / Express proof (New York). The latter retains its publication hold and has no Pages workflow. Each repository owns only its edition build, local data tools, artifacts and regression gates. London and Paris retain their former Gleislicht URLs through static compatibility redirects. Current pins are recorded per release; see the [alpha.15 rollout](CONSOLIDATION-ALPHA-15.md#verified-rollout).
