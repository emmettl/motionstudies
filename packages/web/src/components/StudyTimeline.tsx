...
export function StudyTimeline({ bins, windowStart, windowEnd, time, onSeek, label, ariaLabel = label, description, variant = 'bars', step = 1, maximum, disabled = false, formatTime = formatTimelineTime, formatValue = formatCount, missingLabel = 'No observation', emptyLabel = 'No observations in this window', scaleLabel = 'Scale', onScrubStart, onScrubEnd, className = '' }: StudyTimelineProps) {
  const id = useId(), [hover, setHover] = useState<number>(), scrubbing = useRef(false)
  const model = useMemo(() => createTimelineModel(bins, { start: windowStart, end: windowEnd }, maximum), [bins, windowStart, windowEnd, maximum])
  const position = timelinePosition(time, model.window), value = Number(timelineTime(position, model.window, step).toPrecision(12))
  ...
  return <div className={`ms-study-timeline ${className}`} data-variant={variant} data-disabled={disabled || undefined}>
    ...
    <div className="ms-study-timeline__plot">
      ...
      <input type="range" aria-label={ariaLabel} aria-describedby={description ? `${id}-description` : undefined} aria-valuetext={`${formatTime(value)} · ${reading(active?.value)}`} min={windowStart} max={windowEnd} step={step} value={value} disabled={disabled}
        onChange={event => onSeek(Number(event.target.value))} onPointerDown={begin} onPointerUp={finish} onPointerCancel={finish} onBlur={() => { finish(); setHover(undefined) }} onKeyDown={event => { if (['ArrowLeft', 'ArrowRight', 'ArrowUp', 'ArrowDown', 'Home', 'End', 'PageUp', 'PageDown'].includes(event.key)) begin() }} onKeyUp={finish}
        onPointerMove={event => { if (disabled || event.pointerType === 'touch') return; const box = event.currentTarget.getBoundingClientRect(); setHover(timelineTime((event.clientX - box.left - 9) / Math.max(1, box.width - 18), model.window, step)) }} onPointerLeave={() => setHover(undefined)} />
    </div>
