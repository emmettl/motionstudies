...
/** Problems with a compiled day's manifest, empty when consistent: all 24 hours of the date accounted for once, one
 * slice and one members file per slice, and the file list agreeing with the stated file and byte totals (the manifest
 * itself is the one unlisted file). A trimmed fixture, whose statistics describe the whole day, fails by design. */
export function checkDayManifest(manifest:PublishedDayManifest):string[] {
 const problems:string[]=[],hours=[...manifest.hours.present,...manifest.hours.missing],distinct=new Set(hours).size
 if(hours.length!==24||distinct!==24)problems.push(`Hours present and missing account for ${distinct} distinct of 24, listed ${hours.length} times`)
 if(hours.some(h=>!h.startsWith(`${manifest.date}T`)))problems.push('An hour lies outside the manifest date')
 const count=(prefix:string)=>manifest.list.filter(f=>f.path.startsWith(prefix)).length
 if(manifest.stats.slices!==manifest.slices)problems.push(`Statistics count ${manifest.stats.slices} slices; the manifest ${manifest.slices}`)
 if(count('slices/')!==manifest.slices)problems.push(`${count('slices/')} slice files listed for ${manifest.slices} slices`)
 if(count('members/')!==manifest.slices)problems.push(`${count('members/')} members files listed for ${manifest.slices} slices`)
 if(manifest.list.length+1!==manifest.files)problems.push(`${manifest.list.length} listed files and the manifest disagree with ${manifest.files} files`)
 const bytes=manifest.list.reduce((a,f)=>a+f.bytes,0)
 if(bytes!==manifest.bytes)problems.push(`Listed files hold ${bytes} bytes; the manifest states ${manifest.bytes}`)
 return problems
}
/** Problems reconciling a slice with its members, empty when they agree: the sea and the shoal must count the same
 * vehicles. Each cell's distinct-vehicle count equals its distinct members, no member repeats, and neither file names
 * a cell the other lacks. Samples are not checked here; members carry identities, not sample counts. */
export function reconcileSlice(slice:SliceFile,members:MembersFile):string[] {
 if(slice.slice!==members.slice)return [`Slice ${slice.slice} paired with members of ${members.slice}`]
 const problems:string[]=[],seen=new Set<string>()
 for(const [lon,lat,,vehicles] of slice.cells){
  const key=cellKey(lon,lat),refs=members.cells[key];seen.add(key)
  if(!refs){problems.push(`Cell ${key} counts ${vehicles} vehicles and has no members`);continue}
  const unique=new Set(refs.map(member=>JSON.stringify(member))).size
  if(unique!==refs.length)problems.push(`Cell ${key} repeats a member`)
  if(unique!==vehicles)problems.push(`Cell ${key} counts ${vehicles} vehicles but lists ${unique} members`)
 }
 for(const key of Object.keys(members.cells))if(!seen.has(key))problems.push(`Members name cell ${key}, absent from the slice`)
 return problems
}
