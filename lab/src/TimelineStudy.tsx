...
export function TimelineStudy() {
  ...
  const [scrubbing, setScrubbing] = useState(false), [thinTrack, setThinTrack] = useState(false), [discreetMarker, setDiscreetMarker] = useState(false)
  ...
  return <div style={{ '--ms-timeline-track-height': thinTrack ? '1px' : '8px', '--ms-timeline-marker-size': discreetMarker ? '10px' : '20px', '--ms-timeline-halo-width': discreetMarker ? '0px' : '3px' } as CSSProperties}>
    <div className="toolbar"><label>Chart <select aria-label="Chart style" value={variant} onChange={event => setVariant(event.target.value as 'bars' | 'line')}><option value="bars">Bars</option><option value="line">Line</option></select></label>
      ...
      <label><input type="checkbox" checked={discreetMarker} onChange={event => setDiscreetMarker(event.target.checked)}/> Discreet marker</label>
